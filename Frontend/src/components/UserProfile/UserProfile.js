...
function UserProfile() {
  ...
  async function getOrders() {
    await axios.get(`https://pujari-jcb-spares-backend.onrender.com/orders/getOrders/${userInfo.email}`)
      .then((response) => {
  ...
  async function submitClick() {
    ...
    if (nameError.innerText === '' && passwordError.innerText === '' && securityCodeError.innerText === '') {
      ...
      try {
        let res = await axios.put(`https://pujari-jcb-spares-backend.onrender.com/users/updateUser/${email.value}`, newUserData)
        if (res.status === 200) {
  ...
  async function cancelOrderClick(order) {
    ...
    if (confirmation) {
      try {
        let res = await axios.delete(`https://pujari-jcb-spares-backend.onrender.com/orders/cancleOrder/${order.orderId}`)
        if (res.status === 200) {
          try {
            let res = await axios.get(`https://pujari-jcb-spares-backend.onrender.com/orders/getOrders/${userInfo.email}`)
            if (res.status === 200) {
              setAllOrders(res.data)
              successToastMessage('Order successfully canceled', 3000)
              // setMessage("Order successfully canceled")
              // setOpen(true)
            }
            else {
              errorToastMessage('Something went wrong. Please try again !', 3000)
  ...
  return (
    <>
      ...
      <div className='ms-5 mb-5' style={{ marginTop: '70px', width: '70%' }}>
