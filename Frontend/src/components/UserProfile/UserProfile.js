...
function UserProfile() {
  ...
  async function getDeliveredOrders() {
    try {
      let res = await axios.get(`https://pujari-jcb-spares-backend.onrender.com/orders/get-delivered-orders?email=${userInfo.email}`)
            ...
            <tbody>
              {
                deliveredOrders.length ? deliveredOrders.map((e, i) => {
                  return (<tr key={i}>
                    ...
                    <td>{new Date(new Date(e.date).setDate(new Date(e.date).getDate() + 5)).toLocaleDateString()}</td>
                    <td>
                      <button type="button" id='sumbitbutton' className="btn btn-sm btn-outline-danger" onClick={() => { cancelOrderClick(e) }}>Delete</button>
                    </td>
                  </tr>)
                }) : <tr style={{ width: '100%', fontFamily: 'Arial' }}><td>No Data</td></tr>
              }
