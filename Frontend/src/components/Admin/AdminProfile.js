...
function AdminPage() {
    ...
    useEffect(() => {
        // all users data
        axios.get(`https://pujari-jcb-spares-backend.onrender.com/users?email=${email}`)
            .then((res) => {
        ...
        // all orders data
        axios.get(`https://pujari-jcb-spares-backend.onrender.com/orders?email=${userEmail}&startDateTime=${orderStartDateTime}&endDateTime=${orderEndDateTime}`)
            .then((res) => {
                setOrders(res.data)
            })
            .catch((error) => {
    ...
    async function deleteUserClick(userId) {
        ...
        if (confirmation) {
            try {
                let res = await axios.delete(`https://pujari-jcb-spares-backend.onrender.com/users/deleteUser?userId=${userId}`)
                if (res.status === 200) {
                    try {
                        let res = await axios.get(`https://pujari-jcb-spares-backend.onrender.com/users?email=${email}`)
                        if (res.status === 200) {
                            infoToastMessage('User successfully deleted', 2000)
                            setUserNames(res.data)
                            setUserData(res.data)
                        }
                        else {
    ...
    async function deleteOrderClick(orderId) {
        ...
        if (confirmation) {
            try {
                let res = await axios.delete(`https://pujari-jcb-spares-backend.onrender.com/orders/cancleOrder/${orderId}`)
                if (res.status === 200) {
                    try {
                        let res = await axios.get(`https://pujari-jcb-spares-backend.onrender.com/users?email=${userEmail}&startDateTime=${orderStartDateTime}&endDateTime=${orderEndDateTime}`)
                        if (res.status === 200) {
                            setOrders(res.data)
                            infoToastMessage('Order successfully deleted', 2500)
    ...
    async function deleteProductClick(product) {
        ...
        if (confirmation) {
            try {
                let res = await axios.delete(`https://pujari-jcb-spares-backend.onrender.com/products/deleteProduct/${product.productId}`)
                if (res.status === 200) {
                    try {
                        let res = await axios.get('https://pujari-jcb-spares-backend.onrender.com/products/allProducts')
                        if (res.status === 200) {
                            setAllProducts(res.data)
                            infoToastMessage('Product successfully deleted', 3000)
    ...
    async function saveChangesClick() {
        ...
        if (priceError.innerText === '' && deliveryChargesError.innerText === "") {
            ...
            try {
                let res = await axios.put(`https://pujari-jcb-spares-backend.onrender.com/products/updateProduct/${product.productId}`, productInfo)
                // console.log(res);
                if (res.status === 200) {
                    try {
                        let res = await axios.get('https://pujari-jcb-spares-backend.onrender.com/products/allProducts')
                        if (res.status === 200) {
                            infoToastMessage('Product information successfully updated', 3000)
                            ...
                            <tbody>
                                {
                                    ...
                                    }) : <div style={{ width: '100%', fontFamily: 'Arial' }}><h4>No Data</h4></div>
                                }
                            </tbody>
                        </table>
                    </div>
