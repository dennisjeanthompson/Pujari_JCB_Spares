...
function AdminPage() {
    ...
    useEffect(() => {
        // all users data
        axios.get(`https://pujari-jcb-spares-user.onrender.com?${email}`)
            .then((res) => {
    ...
    async function deleteUserClick(e) {
        ...
        if (confirmation) {
            await axios.delete(`https://pujari-jcb-spares-user.onrender.com/deleteUser/${e.email}`)
                .then((res) => {
                    async function getAllUsers() {
                        await axios.get(`https://pujari-jcb-spares-user.onrender.com?${email}`)
                            .then((res) => {
                                setUserNames(res.data)
                                setUserData(res.data)
    ...
    async function deleteProductClick(product) {
        ...
        if (confirmation) {
            await axios.delete(`https://pujari-jcb-spares-order.onrender.com/deleteProduct/${product._id}`)
                .then((response) => {
        ...
        <>
            ...
            <div className="m-3">
                <div className='' style={{ marginTop: '30px' }}>
                    ...
                    <div className='mt-3 shadow rounded' style={{ width: '70%', height: '230px', overflowY: 'auto', position: 'sticky', top: 0 }}>
                            ...
                            <tbody>
                                {
                                    userData.length ? userData.map((e, i) => {
                                        return (<tr key={i}>
                                            ...
                                            <td>{e.email}</td>
                                            <td>
                                                <button type="button" id='sumbitbutton' className="btn btn-sm btn-outline-danger" onClick={() => { deleteUserClick(e) }}>Delete User</button>
                                            </td>
                                        </tr>)
                ...
                <div style={{ marginTop: '80px' }}>
                    ...
                    <div className='mt-3 shadow rounded' style={{ width: '100%', height: '325px', overflowY: 'auto', position: 'sticky', top: 0 }}>
                            ...
                            <tbody>
                                {
                                    orders.length ? orders.map((e, i) => {
                                        return (<tr key={i}>
                                            ...
                                            <td>
                                                <button type="button" id='sumbitbutton' className="btn btn-sm btn-outline-danger" onClick={() => { deleteOrderClick(e) }}>Delete Order</button>
                                            </td>
                                        </tr>)
                                    }) : <div style={{ width: '100%', fontFamily: 'Arial' }}><h4>No Data</h4></div>
                                }
                ...
                <div className='mt-5'>
                    ...
                    <div className='border rounded p-4' style={{ height: '800px', overflow: 'auto', display: 'grid', justifyContent: 'center', gridTemplateColumns: 'auto auto auto auto', gridGap: '70px' }}>
                        {
