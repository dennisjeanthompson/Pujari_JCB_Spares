...
function ProductDetails() {
    ...
    // axios.get(`https://pujari-jcb-spares-backend.onrender.com/orders?email=&startDateTime=&endDateTime=`)
    //     .then((res) => {
    ...
    async function confirmOrderClick() {
        ...
        if (nameError.innerText === "" && addressError.innerText === "" && mobileError.innerText === "" && quantityError.innerText === "") {
            ...
            try {
                let res = await axios.post('https://pujari-jcb-spares-backend.onrender.com/orders/newOrder', placedOrderData)
                if (res.status === 201) {
                    const emailData = {
                    ...
                    try {
                        let res = await axios.post('https://pujari-jcb-spares-backend.onrender.com/orders/sendEmail', emailData)
                        if (res.status === 201) {
                            setOrderForm(false)
                            successToastMessage('Order Placed ! Please check your email for order details', 3500)
                            // setMessage("Order Placed ! Please check your email for order details")
                            // setOpen(true);
                        }
                        else {
                            setOrderForm(false)
